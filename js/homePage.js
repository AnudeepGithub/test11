...
//function to create the info pods for groups(eg:Public Safety,Transportation,Capital Projects etc.)
/*This function is used to create and style the pods like coloring and indicating the arrow according to their group webmap statistics with their click events*/
function CreateLayerPods(arrSubjectGroups, token, groupdata, indicatorState) {
    ...
    for (var p in arrSubjectGroups) {
        ...
        divPod.onclick = function (evt) {
            /*This event triggers when you click on the pod and it will transforms to map page with the particular map and metric pods*/
        ...
        for (var g in arrSubjectGroups[p]) {
            for (var j in indicatorState) {
                if (arrSubjectGroups[p][g].title == indicatorState[j].key) {
                    ...
                    /*Calculation of statistical information,according to that coloring and styling the pods*/
    ...
    /*Used to create metric(sub group) pods when you share the map*/
    if (share != "") {
        ShowProgressIndicator();
        if (map) {
            for (var e in map._layers) {
...
/*This function is used to get data for metric pods by using their particular webmap Id's*/
        ...
        if (webMapId[id]) {
            ...
            webmapDetails.addCallback(function (response) {
                ...
                if ((webMapId.length - 1) == counter) {
                    ...
                    /* if visibility(isPodVisible) flag set to true in the configuration file,
                    then metric pods will be created,else if set to false,then metric pods will not be created.*/
                    if (visibility) {
                        var webStats = [];
...
/*This function is used to fetch the statistical data by querying particular statistical layers*/
...
//function to fetch the RSS feeds news and twitter trends through local storage
...
//function to fetch the news and events data
/*This function is used to fetch the data for RSS and Trends by using their keywords*/
...
//function to create structure for news and trends
...
/*This function is used to create the map by using their webmap ID*/
...
function MapInitFunction(groupdata, token, webmapInfo, podsVisibility, id) {
    ...
    /*Used to create existing graphics or notes when you share the map with its data by using URL*/
    if (share != "") {
        var group;
        if (window.location.href.split("$n=").length > 1) {
            group = window.location.href.split("$t=")[1].split("$n=")[0];
    ...
    //this function will trigger when you click on the map
...
//function to display info window with notes
/*This function is used to create info window with their existing notes and calculation of characters remaining*/
function ShowNotesInfo(feature, geometry, key, render, note) {
    ...
    infoContentBlur = dojo.connect(txtArea, "onblur", function () {
        /*This function used for calculation of number of characters remain to enter and saving the data on focus out of the control*/
...
//function to save the notes with added comments
